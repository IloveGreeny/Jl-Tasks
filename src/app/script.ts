...
export const getScore = (gameStamps: Stamp[], offset: number): Score | undefined => {
    ...
    const stamp = gameStamps.reduce((maxStamp, currentStamp) => {
        return currentStamp.offset <= offset && currentStamp.offset > maxStamp.offset? currentStamp : maxStamp;
    }, { offset: -Infinity, score: { home: 0, away: 0 } });
    return stamp.score;
};
